...
Before LEOptical can send a single email from Marketing Cloud Next, you need to configure domain authentication. This is not a "nice to have." Marketing Cloud Next Growth and Advanced editions require a DKIM-authenticated domain as the sender address. There is no default sending domain in an SDO. If you skip this, email sending is blocked.

This module covers three distinct domain configurations: the authenticated sending domain (what appears in the `From:` address), the link branding domain (what click-tracking URLs look like), and the landing page domain (what URL your Marketing Cloud Next landing pages serve from). Each one is a separate subdomain with its own DNS records and its own setup path. They can all live on the same root domain, but they cannot share a subdomain.

If you are coming from MCE, the most important thing to know upfront is that Marketing Cloud Next has no SAP. What SAP bundled together in MCE (authenticated domain, link branding, RMM) is now configured individually through a self-service Setup UI. No procurement step. No Salesforce support ticket. You do it yourself.
...
- Why Marketing Cloud Next requires domain authentication before any email can send.
- The three domain roles in Marketing Cloud Next and how they differ from each other.
- How to configure an authenticated sending domain, including the DNS records IT needs to create.
- What DMARC is, why Marketing Cloud Next does not configure it for you, and how to add it yourself.
...
- How Marketing Cloud Next's domain setup compares to MCE's Sender Authentication Package (SAP).

## Why Domain Setup Comes Before Everything Else

Marketing Cloud Next enforces DKIM authentication at send time. You cannot use an unverified domain as the sender address (source: cgc-agency.com, SFMC Tips #304). This applies to both the Growth and Advanced editions. The SDO does not ship with a pre-configured sending domain, so there is nothing to inherit.
...
Marketing Cloud Next uses three separate domains for three separate purposes. They are not interchangeable. Each one is a different subdomain pointing at different Salesforce infrastructure.
...
| Landing page domain | Serves Marketing Cloud Next-hosted landing pages. | `pages.leoptical.com` | Free via Salesforce CDN (Cloudflare) |
...
- Subdomain delegation (handing nameserver control to Salesforce) is **not** supported in Marketing Cloud Next Growth or Advanced. It was available in legacy MCE. Your IT team must add each DNS record manually at the registrar.
- DKIM keys are 2048-bit by default in Marketing Cloud Next. MCE defaulted to 1024-bit.
:::
...
2. [Landing Page Domain](./landing-page-domain): configure a branded subdomain for Marketing Cloud Next-hosted landing pages using the free Salesforce CDN option.
