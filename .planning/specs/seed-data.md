...
|------|---------|-----------|-------|
| `loyalty_members.csv` | ~40,471 | Loyalty Program Member | Uses loyalty IDs. Sometimes different email than CRM. Sets up IDR exercise. |
| `ecommerce_orders.csv` | ~98,314 orders (~147k rows) | Sales Order + Sales Order Product | Denormalized. References products by SKU. Order emails may differ from CRM. Includes protagonist orders. |
| `exam_history.csv` | ~33,829 | Eye Exam (custom DMO) | Patient emails for IDR. Mixed date formats (dirty data). |
...
|---------|---------------|---------------------|
| Different emails, same person | CRM has `maria.chen@work.example.com`, loyalty CSV has `m.chen@personal.example.com`, ecommerce has `maria.chen@other.example.com` | Every multi-source client |
...
| Stale data | ~500 loyalty members with tier "Gold" in CSV but points below the Gold threshold | Snapshot vs. real-time data lag |
