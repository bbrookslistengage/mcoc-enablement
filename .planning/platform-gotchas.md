...
`sidebar_label` frontmatter is supported in Docusaurus 3.x autogenerated sidebars. It accepts any string value, including those with Unicode character prefixes. A Unicode prefix (e.g., `▶`) can be prepended to the label value to visually distinguish Project pages from lesson modules without switching from autogenerated to explicit sidebar config in `sidebars.ts`. Applied to `docs/part-1-foundations/consent-automation-flow.md` as `sidebar_label: "▶ Consent Automation Flow"`.
